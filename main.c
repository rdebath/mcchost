...
void
login_check_packet(pkt_player_id *player_p)
{
    char inbuf[4096] = {0};
...
{
    if (!user || *user == 0) disconnect(0, "Username must be entered");
...
login()
{
	...
	if (check_mppass(player.user_id, player.mppass)) {
		...
		disconnect(0, "Login failed! Please connect using the server list.");
