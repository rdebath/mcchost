...
#if INTERFACE
#define UCMD_REPLACE \
    {N"replace", &cmd_replace, .map=1}, {N"r", &cmd_replace, CMD_ALIAS}, \
    {N"replaceall", &cmd_replaceall, .map=1, CMD_PERM_LEVEL}, {N"ra", &cmd_replaceall, CMD_ALIAS}
#endif
...
cmd_replace(char * cmd, char * arg)
{
    ...
    int blcount = 0;
    for(char * ar = strarg(arg);ar;ar=strarg(0)) {
